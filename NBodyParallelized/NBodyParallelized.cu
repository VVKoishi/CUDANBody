...
__device__ float3
tile_calculation(float3 myPosition, float3 accel)
{
	int i;
	extern __shared__ float3 shPosition[];	// shared memory

	#pragma unroll 128						// loop optimization
...
calculate_forces(Body* ptr, float dt, int N)
{
	...
	int p = blockDim.x;	// p thread -> p body interactions per tile
	int gtid = blockIdx.x * blockDim.x + threadIdx.x; // this thread/body index
	float3 myPosition = { ptr[gtid].x, ptr[gtid].y, ptr[gtid].z }; // each body self pos
...
int main(const int argc, const char** argv) {
    ...
    int sharedMemSize = blockSize * 3 * sizeof(float); // 3 floats for pos

	for (int iter = 0; iter < nIters; iter++) {
		calculate_forces <<< numBlocks, blockSize, sharedMemSize >>> (p, dt, nBodies);
